...
tip_positions = collections.deque(maxlen=5)  # Stores last 5 tip positions
            ...
            if len(tags)>0:
                ...
                if x is not None and y is not None:
                    ...
                    if len(tip_positions) > 1:
                        # Compute motion vector (current - previous)
                        dx = tip_positions[-1][0] - tip_positions[0][0]
                        dy = tip_positions[-1][1] - tip_positions[0][1]

                        # Normalize direction
                        norm = max(np.linalg.norm([dx, dy]), 1e-6)  # Avoid division by zero
                        dx, dy = dx / norm, dy / norm  
                        speed = np.linalg.norm([dx, dy])  # Speed magnitude
                        scale = min(100, speed * 10)  # Adjust vector length based on speed

                        # # Extend the tip in this direction
                        scale = 100  # Length of the extended tip
                        x = int(x + dx * scale)
                        y = int(y + dy * scale)
                        # color = (0, 255, 0) if speed < 10 else (0, 0, 255)  # Green for slow, Red for fast
                        # Draw vector
                        cv2.line(frame, (int(x), int(y)), (x, y), (0,0,255), 10)
